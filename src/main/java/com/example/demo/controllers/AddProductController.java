...
//        product.getParts().addAll(assparts);
            ...
            if(product.getId()!=0) {
                ...
                for (Part p : product2.getParts()) {
                    int newInv = p.getInv() - (product2.getInv() - product.getInv());
                    if (newInv - 1 < p.getMinInventory()) {
                        bindingResult.rejectValue("inv", "inventory.invalid", "Adding/updating this product would lower part inventory below the minimum.");
                        theModel.addAttribute("parts", partService.findAll());
                        List<Part> availParts = new ArrayList<>();
                        ...
                        theModel.addAttribute("availparts", availParts);
                        theModel.addAttribute("addparts", product2.getParts());
                        theModel.addAttribute("assparts",product2.getParts());
                        return "productForm";
                    }
                }
